	...
	struct RegretAutomaton {
		...
		struct TransformComparer {
			bool operator()(const Transform &Ta, const Transform &Tb) const {
				return std::tuple<int,int,int,int>(ita(Ta), Ta.i, Ta.B, Ta.E) > std::tuple<int,int,int,int>(ita(Tb), Tb.i, Tb.B, Tb.E);
			}
		};

		using Heap = priority_queue<Transform,std::vector<Transform>, TransformComparer>;
		...
		struct Chooser {
			...
			std::vector<int> DP(std::vector<Transform> ca) {
				...
				std::vector<std::vector<std::pair<int, int>>> parent(
				    groups.size(),
				    std::vector<std::pair<int, int>>(size, {-2, -2})
				);
				dp[offset] = 0;
			...
			std::vector<int> MITM(const std::vector<Transform> &ca) {
				...
				auto dfsLeft = [&](auto &&dfsLeft, int idx,
				                   const std::array<int, D> &curVec,
				ll cost, std::vector<int> &combo) {
				...
				auto dfsRight = [&](auto &&dfsRight, int idx,
				                    const std::array<int, D> &curVec,
				ll cost, std::vector<int> &combo) {
		...
		ll solve(int targetMain) {
			...
			while (true) {
				...
				bool hasCache = (sec.cachedI.size() == candidates.size() &&
				                 sec.cachedC.size() == candidates.size());
				ll delcost = 0;
		...
		RegretAutomaton(int n_,
			...
			: N(n_),
			  ...
			  curVals{},
			  Delta(maxDeltaAbs_),
			  getTransitions(getTransitions_),
			  sec(*this) {
			for (int i = 0; i < N; ++i) {
				insertTransitions(i);
...
int main() {
	...
	for (int i = 0; i < k && sb.size(); ++i) {
		auto it = --sb.end();
		st[(*it)[1]] |= 2;
		sb.erase(it);
	}
	auto *ra = new RegretingGreed::RegretAutomaton<3, 1, ita, C>(n, st, getTran, 2);
	cout << -ra->solve(l) << endl;
	delete ra;
	return 0;
}
